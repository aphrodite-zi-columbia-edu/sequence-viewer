...
function _compileDefaultConfiguration(defaultLocation, userLocation) {
  ...
  locations.forEach(function (location) {
    var locationName = nameReverseMap[location] || location;

    try {
      newData = yaml.load(fs.readFileSync(location, 'utf8'));
      logger.debug("Loaded data from", locationName);

      defaults = Object.assign(defaults, newData);
      logger.trace("Configuration:", defaults);
    } catch (e) {
      ...
      logger.debug("Failed to load from", locationName);
    }
  });
...
var compileDefaultConfiguration = _compileDefaultConfiguration.bind(
  ...
  path.join(process.cwd(), '.sequence-viewerrc')
);
var nameReverseMap = {
  [path.join(__dirname, '..', '..', 'data', '.sequence-viewerrc')] : 'data',
  [path.join(homedir(), '.sequence-viewerrc')]                     : 'home',
  [path.join(process.cwd(), '.sequence-viewerrc')]                 : 'cwd',
};
...
var experimentConnectors = {
  csv: function (filesField, eS) {  // experimentsSource
    try {
      ...
      logger.trace("CSV experiment connector found file data", data[0][filesField], "in col:", filesField);
      return true;
    } catch (err) {
      console.log(err);
      return false;
